...
module.exports = ({ app, pool, config, authorise }) => {
  app.get('/api/domains', authorise, (req, res) => {
    pool.query('select * from "domain" where user_id = $1 or system = 1', [req.user.id], (err, res) => {
      if (err) {
        console.error('failed to get domains', err)
        return res.status(500).send({ error: 'Unxpected Error' })
      }
      const domains = res.rows.map(prepareDomain)
      res.send({ domains })
    })
  })
  ...
  app
    ...
    .get(authorise, (req, res) => {
      pool.query(
        'select * from "domain" where (user_id = $1 or system = 1) and id = $2',
    ...
    .delete(authorise, (req, res) => {
      pool.query('delete from "domain" where user_id = $1 and id = $2', [req.user.id, req.params.id], (err, res) => {
        if (err) {
